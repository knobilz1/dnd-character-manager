...
- **Converting a printed label like "F4" into `{q, r}`: count from ZERO.** The labels start at A and 1, but `q` and `r` start at 0, so **A1 is `{"q": 0, "r": 0}`** — subtract one from the row number, and count the column letter with A=0, B=1, C=2… So C3 is `{"q": 2, "r": 2}`, F4 is `{"q": 5, "r": 3}`, and K3 is `{"q": 10, "r": 2}`. Getting this wrong puts a creature or a spell one square up and to the left of where everyone at the table is looking — the kind of error nobody notices until it decides who got caught in a fireball.
- **That conversion is ONLY for labels you are reading off a map.** A `{q, r}` that is already written as numbers — in the battle log, in a Deployment line, in your own earlier action — is finished; copy the digits exactly as they stand. Never subtract one from a number that is already a `q` or an `r`. If the ogre's `coord` reads `{"q": 11, "r": 5}`, then a spell centred on the ogre is `{"q": 11, "r": 5}`.
...
- **No map for where the fight is heading? Ask for one EARLY.** Set the `makeMap` dm-action to ONE LINE describing the place and the fight — `"makeMap": "bandit ambush on the rope bridge over the foggy gorge"` — and a battle map is built for it. Use it when battle_maps/index.md has nothing for where the fight is going; use `recallMap` instead when it already does. It takes several minutes and nothing waits on it: you keep running the scene and the map reaches the table when it's done. So the useful moment is when the fiction first leans toward a fight — the party decides to storm the mill, a patrol spots them, someone starts sharpening a knife — NOT when you are about to roll initiative. A map that arrives after the fight helped nobody. One request per fight, and only for a fight actually taking shape: each one is minutes of real work, so don't ask speculatively for everywhere the party might wander. **Asking counts as a reason to emit a dm-actions block.** Nothing has "changed" when you ask for a map, so a turn where the party simply decides to go and fight somewhere — and nothing else happened — still ends with a block containing only `makeMap`. That decision IS the trigger; waiting for something else to change means waiting until it is too late.
- **Open the fight from the map's `Deployment:` section.** Its `Enemies:` line is a per-miniature placement list — `Goblin Boss at K3; Goblin at J2, L2, M4` — so it already tells you the roster AND the square each creature stands on: one listed cell is one creature. Seed the Active Battle Log straight from it, each creature's cell becoming its `coord`, and field exactly the creatures it names rather than inventing a different number. The `Party:` line is a zone instead, since the players pick their own squares inside it — take each PC's actual square from the players. It's a starting suggestion you may bend to the fiction (a stealthy party might come in elsewhere), but it's how the map tells you where round one begins, so you're tracking real positions from the first initiative count.
...
- **Put lasting spell areas ON the map.** When a spell creates an area that PERSISTS — Web, Darkness, Cloudkill, Fog Cloud, Spike Growth, Moonbeam, Flaming Sphere, Wall of Fire, Silence, Entangle, Hunger of Hadar — send `placeEffect` with the spell and the square it's centred on — inside your usual dm-actions block, as a TOP-LEVEL key sitting beside `battleLog` rather than a field inside it: `{"placeEffect": [{"spell": "web", "at": {"q": 6, "r": 2}}], "battleLog": {…}}`. **Centre it on what the player actually named.** "Web right on that kobold" means the centre square IS the kobold's square — copy that creature's `coord` from the battle log exactly. Give the spell's id or its exact name and the centre cell only — never a radius, never a pixel size, never a list of covered squares: the app looks the real shape and size up from the spell itself, so you cannot get the geometry wrong by sending it. For a cone or a line, add `"angle"` in degrees (0 points along +q, i.e. toward higher column letters, and it turns clockwise).
  - **Only for spells that LAST.** A Fireball is instantaneous — it detonates and is gone, so it must never be placed; an area that stays on the table where a one-shot blast happened is worse than nothing. If the spell's duration is Instantaneous, narrate it and send no effect.
  - **Move it when it moves.** Flaming Sphere gets shoved around as a bonus action: `"moveEffect": [{"id": "flaming-sphere-1", "to": {"q": 8, "r": 4}}]`. The id is the one shown back to you in the battle log. Only ever move an effect that is ALREADY on the map with that exact id — the first time a spell appears it is a `placeEffect`, even when the player describes it as ramming or driving the thing somewhere. A `moveEffect` naming an id that was never placed changes nothing on the map, so the area you just narrated silently isn't there.
  - **Take it away when it ends early.** Concentration broken, dispelled, or the caster drops it: `"removeEffect": ["web-1"]`. You do NOT need to remove one that simply ran out of rounds — the app expires those on its own clock.
  - These are environment, never creatures. Never place an effect to represent a monster, an NPC or a character — the minis on the table are the creatures.
"##;
...
",
    ...
    format!(
        ...
        - Every `Objects:` entry must EARN its place: it should be cover, an obstacle, difficult terrain, wall storage, or a detail the Tactics or the fiction actually lean on. Don't scatter a prop with no reason to be there — a lone net, a random barrel, a stray crate dropped mid-floor reads as clutter, not atmosphere. If you can't say what it does or why it's there, leave it out. A prop that only reads WITH a companion is worse than nothing alone: a fishing net, a single oar, a lone horseshoe, a coil of rope means nothing dropped in open floor. Place one ONLY touching the thing it belongs to — a net on or beside a drying rack or a boat, an oar in a boat, a horseshoe by a forge — or leave it out.\n\
...
const DEPLOYMENT_FORMAT_LINE: &str = "Deployment:\n\
    - Enemies: <one entry per miniature, \"<creature> at <cell>\", entries separated by \";\", e.g. \"Goblin Boss at K3; Goblin at J2, L2, M4; Worg at M6\"> — <why there, in prose, naming NO further cells>\n\
    - Party: <the cells the party starts on, e.g. \"B4-D6\"> — <why there, in prose, naming NO further cells>\n";
...
fn battle_map_format_instructions_full(objects_enabled: bool, vocabulary: &[String], footprint_guide: &[String]) -> String {
    ...
    format!(
        ...
        - CHECK THE LAST WORD OF EVERY NAME AGAINST THE VOCABULARY LIST BELOW. That last word is the one the art is looked up by, and a word that isn't in the list finds nothing — the art then falls back to whatever the earlier words happen to match, which is usually something absurd. If your noun isn't listed, rename it to the nearest one that is: the thing itself doesn't change, only the word you file it under. Worked examples from this library — \"Shop counter\" has no `counter`, so it drew a hanging shop SIGN, while \"Shop table\" or \"Market table\" draws the laid trading table you meant; \"Wooden counter\" drew a STAIRCASE. (\"Bar counter\" is fine as written, because `bar` is itself in the list.)\n\
        ...
        - And the reverse: every object cell you draw needs a `Features:` line covering it. An `=` or `o` with nothing naming it has no name for the art to be generated from, so it falls back to exactly the generic look the rule above warns about. Naming it in `Tactics:` does NOT count — that text is prose and is never read for art. Live: a charnel house drew its flesh altar as a 4x2 `=` block and a pillar at M9, named both in Tactics only, and rendered the room's centrepiece as anonymous crates.\n\
...
- **Bone pile I4-J5** is the objective; Drow Darkness poisons the ambush.";
    ...
    fn dm_rules_carry_only_the_active_battle_mode_and_maps_only_on_grid() {
        ...
        // makeMap is documented ONLY here, never in BASE_CLAUDE_MD. That file is
        // written once when a campaign is created and never rewritten, so an
        // action added there reaches new campaigns and no existing one — the
        // feature would be silently dead for every campaign that already exists.
        // dm_rules.md is regenerated on every load, which is also what grid-gates
        // it: Theater has no grid and Hex has its own axial coordinates, so a
        // lettered map suits neither (DMConsolePage enforces the same gate).
        assert!(grid.contains("makeMap"), "grid must teach how to ASK for a map it doesn't have");
        assert!(!theater.contains("makeMap"), "no map building off Grid: {theater}");
        assert!(!hex.contains("makeMap"), "no map building off Grid: {hex}");
