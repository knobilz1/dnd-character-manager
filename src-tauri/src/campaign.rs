...
- **Put lasting spell areas ON the map.** When a spell creates an area that PERSISTS — Web, Darkness, Cloudkill, Fog Cloud, Spike Growth, Moonbeam, Flaming Sphere, Wall of Fire, Silence, Entangle, Hunger of Hadar — send `placeEffect` with the spell and the square it's centred on: `"placeEffect": [{"spell": "web", "at": {"q": 6, "r": 2}}]`. Give the spell's id or its exact name and the centre cell only — never a radius, never a pixel size, never a list of covered squares: the app looks the real shape and size up from the spell itself, so you cannot get the geometry wrong by sending it. For a cone or a line, add `"angle"` in degrees (0 points along +q, i.e. toward higher column letters, and it turns clockwise).
  ...
  - **Move it when it moves.** Flaming Sphere gets shoved around as a bonus action: `"moveEffect": [{"id": "flaming-sphere-1", "to": {"q": 8, "r": 4}}]`. The id is the one shown back to you in the battle log.
